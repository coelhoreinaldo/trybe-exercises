...
let repeatedFruit = null;
let repeats = null;
let repeatedFruitsIndex = null;

function totalFruits(array) {
    // let divideArray = array.join(',');
    for (let fruits in basket) {
        let verificaFruits = basket[fruits];
        for (let fruits2 in basket) {
            if (basket[fruits2] === verificaFruits){  
                repeats += 1;
                repeatedFruit = basket[fruits2];
                repeatedFruitsIndex = fruits2;            
            }
            return "Sua cesta possui: ", repeats, repeatedFruit
        }
    }
    
}

console.log(totalFruits(basket));